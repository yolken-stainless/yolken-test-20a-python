...
from yolken_test_20a import YolkenTest20a, AsyncYolkenTest20a, DefaultAioHttpClient
from yolken_test_20a._utils import is_dict
...
logging.getLogger("yolken_test_20a").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest20a]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest20a(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest20a]:
